...
local function update_contents(blocks, shift_by, include_path)
  local update_contents_filter = {
    ...
    Image = function (image)
      if (string.sub(image.src,1,7) ~= "http://") and (string.sub(image.src,1,8) ~= "https://") and path.is_relative(image.src) then 
  ...
  for line in cb.text:gmatch('[^\n]+') do
    if line:sub(1,2) ~= '//' then
      ...
      if not fh then
        ...
        fh:seek("set",0)
        txt=fh:read("*a")
        if (string.sub(line,-4) == ".csv") then
          txt=string.gsub(txt,",","|")
          n=string.find(txt,"\n")
          line1=string.sub(txt,1,n)
          txt= line1 .. string.gsub(line1,"[^|^\n]","-") .. string.sub(txt,n+1,-1)
        end
        local contents = pandoc.read(txt, format, PANDOC_READER_OPTIONS).blocks
